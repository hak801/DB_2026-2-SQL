...
--컬럼(속성) 리스트를 원해 테이블 컬럼 순서대로 모두 선택
select * from 고객;
...
--가상컬럼명을 설정
--제품테이블에서 제조업체의 중복데이터를 제거하고 검색하시오(단, 제조업체를 대표업체로 가상컬럼명으로 출력)
select distinct 제조업체 as 대표업자 from 제품;
select distinct 제조업체 대표업자2 from 제품;

--제품테이블에서 제품명, 단가를 검색하되, 단가에 500원을 더해서 조정 단가라는 가상칼럼명으로 출력하시오
select 제품명, 단가+500 "조정 단가" from 제품;
select 제품명, 단가, 단가+500 "조정 단가" from 제품;

--where(조건)절
--제품테이블에서 제조업체가 한빛제과인 테이터행을 출력하시오(단, 제품명, 단가, 제조업체 속성만 검색)
select 제품명, 단가, 제조업체
        from 제품
        where 제조업체='한빛제과';
...
--고객테이블에서 고객이름에 "용"글자가 포함된 고객의 고객이름, 나이를 출력하시오
select 고객이름, 나이
        from 고객
        where 고객이름 like '%용%';
        
--고객테이블에서 고객아이디에 "p"가 포함된 고객의 고객아이디, 고객이름을 출력하시오
 select 고객아이디, 고객이름
        from 고객
        where 고객아이디 like '%p%';
        
--고객테이블에서 고객아이디가 "pe"로 시작하고 4개의 글자로 된 고객의 고객아이디, 고객이름을 출력하시오
select 고객아이디, 고객이름
        from 고객
